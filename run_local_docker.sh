...
# Script to build and run the Test Serper API MCP Server locally in Docker
...
IMAGE_NAME="test-serper-api-mcp-server"
CONTAINER_NAME="test-serper-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Test Serper API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Test Serper API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^SERPER_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Test Serper API API key:${NC}"
...
echo
echo -e "${GREEN}✅ Test Serper API MCP Server is running!${NC}"
echo
